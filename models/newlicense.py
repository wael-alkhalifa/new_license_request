...
class nia_new_license(models.Model):
    ...
    order_number = fields.Integer(default=lambda self: self.env['ir.sequence'].next_by_code('increment_new_license'))
    ...
    def print_payment(self):
       # data['form'].update(self.read(['date_from','date_to','report_type','request_type','project','name','date_now','section'])[0])  
        return self.env.ref('decisions.print_payment_doc').report_action(self)    
    
    def print_license(self):
       # data['form'].update(self.read(['date_from','date_to','report_type','request_type','project','name','date_now','section'])[0])  
        return self.env.ref('decisions.print_new_license').report_action(self)       

    def get_license_number(self):
        
        self.license_number = str(self.order_number)+'/'+self.section.name+'/'+str(self.license_yaer)
